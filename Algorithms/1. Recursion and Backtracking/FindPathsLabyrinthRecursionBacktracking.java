...
public class FindPathsLabyrinthRecursionBacktracking {
    ...
    public static void main(String[] args) {
        ...
        char[][] labyrinth = new char[rows][col];

        for (int row = 0; row < rows; row++) {
            labyrinth[row] = scanner.nextLine().toCharArray();
        }
        findPath(labyrinth, 0, 0, 'S');
    }
    ...
    private static void findPath(char[][] labyrinth, int row, int col, char direction) {
        if (!isInBounds(labyrinth, row, col)
                || labyrinth[row][col] == 'V'
                || labyrinth[row][col] == '*') {
            // prevent going in that direction
            return;
        }
        ...
        // after the call we can add backtracking
