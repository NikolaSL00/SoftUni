# Valentine’s Day is coming, and Cupid has very limited time to spread some love across the
# neighborhood. Help him with his mission!
...
# If the needed hearts for a certain house become equal to 0 , print on the console "Place {houseIndex}
# has Valentine's day." If Cupid jumps to a house where the needed hearts are already 0,
# print on the console "Place {houseIndex} already had Valentine's day.".
# Keep in mind that Cupid can have a bigger jump length than the size of the neighborhood and if he does
# jump outside of it, he should start from the first house again.
...
# Next he jumps again with a length of 2 and goes outside the neighborhood, so he goes back to the first house
...
even_integers = [int(el) for el in input().split("@")]
...
while command != "Love!":
    not_love_command = command.split(" ")
    jump_size = int(not_love_command[1])

    if position_index + jump_size < len(even_integers):
        position_index += jump_size
    else:
        position_index = 0
    if even_integers[position_index] > 2:
        even_integers[position_index] -= 2
    elif even_integers[position_index] == 2:
        even_integers[position_index] -= 2
        print(f"Place {position_index} has Valentine's day.")
    elif even_integers[position_index] == 0:
