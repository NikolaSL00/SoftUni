...
from GUI_SHOP.global_constants import DB_FOLDER_NAME, PRODUCTS_IMAGES_FOLDER_NAME
from GUI_SHOP.products_service import get_all_products
...
def render_register_screen(window: tk.Tk):
    ...
    tk.Button(window, text="Register",
              bg='yellow',
              fg="black",
              command=lambda: on_register()).grid(row=4, column=0)
...
def render_product_screen(window: tk.Tk):
    ...
    for idx, product in enumerate(all_products):
        ...
        load_image(col_idx, product, row_idx)

        tk.Label(window, text=product["count"]).grid(row=row_idx + 2, column=col_idx)

        tk.Button(window, text="Buy",
                  bg='black',
                  fg="white").grid(row=row_idx + 3, column=col_idx)
        col_idx += 1


def load_image(col_idx, product, row_idx):
    image = Image.open(os.path.join(DB_FOLDER_NAME, PRODUCTS_IMAGES_FOLDER_NAME, product['image'])
                       ).resize((100, 100))
