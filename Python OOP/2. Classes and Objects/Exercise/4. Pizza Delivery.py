...
class PizzaDelivery:
    ...
    def remove_ingredient(self, ingredient:str, quantity:int, price_per_quantity):
        if self.ordered == True:
            return f"Pizza {self.name} already prepared, and we can't make any changes!"
    ...
    def make_order(self):
        self.ordered = True
        return f"You've ordered pizza {self.name} prepared with {', '.join([ f'{ingr}: {quant}' for ingr, quant in self.ingredients.items()])} and the price will be {self.price}lv."
